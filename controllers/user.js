...
import { JWT_SECRET,JWT_EXPIRATION } from "../config/envConfig.js";
...
const signupUser = async (req, res) => {
  try {
    ...
    bcrypt.hash(password, 10, async (err, hash) => {
      ...
      const user = new User({ name, email, password: hash, role, otp, otpExpiresAt: Date.now() +  10 * 60 * 1000 });
      await user.save();
...
export { getUsers, signupUser, loginUser, changePassword,getUser };